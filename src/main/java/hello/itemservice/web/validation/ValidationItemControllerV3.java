...
//    @PostMapping("/add")
    public String addItemV1(@Validated @ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        // 검증기를 따로 설정하지 않아도 global bean validator 적용
        // Spring 은 @ModelAttribute 에 타입 변환이 성공하여 바인딩이 성공한 필드만 bean validation 적용
        // 바인딩이 실패한 경우, typeMismatch FieldError 추가
        // 오류 코드는 valid 에 사용된 annotation 이름으로 등록됨

        // 특정 필드가 아닌 복합 룰 검증
        // Entity class 에 사용하는 ScriptAssert 는 기능이 빈약하고 복잡한 룰을 처리하기 힘듬.
        // ObjectError 는 직접 코드로 검증 처리, 따로 메서드로 추출해서 사용
